...
/* Revision: 1.37.1.3  BY: Shoma Salgaonkar    DATE: 06/03/05   ECO: *P3NN*   */
/* Revision: 1.37.1.4  BY: Ajit Philip         DATE: 10/11/06   ECO: *P3WX*   */
...
define variable iAPICmtSeq    as integer no-undo.
define variable iAPIMstCmtSeq as integer no-undo.
define variable cAPIOperation as character no-undo.
define variable cAPILang      as character no-undo.
define variable cAPIType      as character no-undo.
define variable cAPIPrint     as character no-undo.
define variable cAPIRef       as character no-undo.
define variable cAPIComment   as character extent 15 no-undo.
define variable lLegacyAPI    as logical no-undo.

/* Transaction Comments API dataset definition */
{gpdscmt.i "reference-only"}
...
if c-application-mode = "API" then do:
   ...
   if valid-handle(ApiMethodHandle) then do:
      /* Get the Transaction Comments API dataset from the controller */
      run getDataset in ApiMethodHandle (input "dsTransactionComment",
                                         output dataset dsTransactionComment bind).

      lLegacyAPI = false.
   end.
   else do:

      /* Get handle of API controller (legacy) */
      {gprun.i ""gpaigh.p""
...
if c-application-mode <> "API" then do:
   ...
   repeat with frame cmmt01:
      ...
      if     lookup(file_name,"so_mstr,sod_det") > 0
         and global_part                        <> ""
         and lookup(global_part,"MAN,MBU,SBU")   > 0
      then do:
         emt-bu-lvl = global_part.
         if    (emt-bu-lvl  <> cmt__qadc01)
            and cmt__qadc01 <> ""
         then do:
            /* COMMENTS ARE READONLY. CREATED AT # */
            {pxmsg.i &MSGNUM=4612 &ERRORLEVEL=1 &MSGARG1=cmt__qadc01}
         end. /* IF (emt-bu-lvl  <> cmt__qadc01) AND ... */
      end. /* IF LOOKUP(file_name,"so_mstr,sod_det") > 0 AND ... */

      set1:
      do on error undo, retry:

         if      (lookup(file_name,"so_mstr,sod_det") > 0
            and  (emt-bu-lvl  = cmt__qadc01
               or cmt__qadc01 = ""))
   ...
   *Condition key:
   ...
   Unknown ttTransComment.seq / ttTransactionComment.cmtSeq values
   ...
   */
   ...
   repeat:
      ...
      if lLegacyAPI then do:
         ...
         /* Get the next comment from the API Controller */
         run getNextRecord in ApiMethodHandle
            (input "ttTransactionComment").
      ...
      if lLegacyAPI then do:
         assign
            iAPICmtSeq    = ttTransComment.seq - 1
            iAPIMstCmtSeq = ttTransComment.masterCmtSeq - 1
            cAPILang      = ttTransComment.lang
            cAPIType      = ttTransComment.type
            cAPIPrint     = ttTransComment.print
            cAPIRef       = ttTransComment.ref
            cAPIOperation = ttTransComment.operation no-error.
         do i = 1 to 15:
            cAPIComment[i] = ttTransComment.cmmt[i].
         end.
      end.
      else do:
         assign
            iAPICmtSeq    = ttTransactionComment.cmtSeq
            iAPIMstCmtSeq = ttTransactionComment.masterCmtSeq
            cAPILang      = ttTransactionComment.cmtLang
            cAPIType      = ttTransactionComment.cmtType
            cAPIPrint     = ttTransactionComment.cmtPrint
            cAPIRef       = ttTransactionComment.cmtRef
            cAPIOperation = ttTransactionComment.operation.
         do i = 1 to 15:
            cAPIComment[i] = ttTransactionComment.cmtCmmt[i].
         end.
      end.
      for first cmt_det
        where cmt_det.cmt_domain = global_domain
          and cmt_indx = cmtindx
          and cmt_seq  = iAPICmtSeq
      exclusive-lock:
      end.

      if cAPIOperation = {&REMOVE} then do:
         if available cmt_det then do:
            delete cmt_det.
         end.
         else if iAPICmtSeq = ? then do:

            if can-find(first cmt_det where
                           cmt_det.cmt_domain = global_domain and
                           cmt_indx = cmtindx)
            then do:
               for each cmt_det
                  where cmt_det.cmt_domain = global_domain
      ...
      then do:
         if lLegacyAPI then do:
            ...
            {pxmsg.i
               &MSGNUM = 4807
               &ERRORLEVEL = 3
               &MSGARG1 = ""string(cmt_seq)""
               &MSGARG2 = ""ttTransactionComment""
               }
            return error.
         end.
      end.
      ...
      if
         (cAPIType <> ? and not validType(cAPIType))
         or
         (cAPIType = ? and not validType(""))
      then do:
         if lLegacyAPI then do:
            /* Invalid # value in record # of # temp-table */
            ...
            return error.
             ...
             /* Invalid # */
            {pxmsg.i &MSGNUM = 3900 &ERRORLEVEL = 3
               &MSGARG1 = getTermLabel(""TYPE"",8)
               }
            return error.
         end.
         ...
         */
         if cAPIPrint <> ? and
            cAPIPrint <> ""
         then do:
            do i = 1 to num-entries(cAPIPrint):
               if lookup(entry(i,cAPIPrint),
                  validCmtPrintList
                  ) = 0
               then do:
                  if lLegacyAPI then do:
         ...
         if cAPIOperation = {&ADD} or
           (cAPIOperation = {&SYNC} and not available cmt_det)
         then
         ...
         */
         if cAPIPrint <> ? then do:

            /* Add entries in cAPIPrint to cmtPrint */
            do i = 1 to num-entries(cAPIPrint):
               if lookup(entry(i,cAPIPrint),cmtPrint) = 0
               then
                  cmtPrint = cmtPrint
                             + ","
                             + entry(i,cAPIPrint).
            ...
            do i = 1 to num-entries(validCmtPrintList):
               ...
               if lookup(validPrintEntry,cAPIPrint) = 0
         ...
         if commentTextProvided = no then do:
            ...
            if iAPIMstCmtSeq <> ? then do:

               for first cd_det
                  where cd_det.cd_domain = global_domain
                    and cd_ref  = cAPIRef
                    and cd_type = cAPIType
                    and cd_lang = cAPILang
                    and cd_seq  = iAPIMstCmtSeq
               no-lock:
               end.
         ...
         */
         ...
         renumberIncrement = 0.
         if iAPICmtSeq <> ? then do:

            /* Page number to use when creating a new page */
            newPageNumber = iAPICmtSeq.

            if cAPIOperation = {&ADD} and available cmt_det
            then do:
         ...
         if renumberIncrement > 0 then do:
            ...
            for each cmt_det
               ...
               cmt_seq = cmt_seq + renumberIncrement.
            end.
         ...
         then do:
            ...
            assign
               {mfaiset.i cmt_ref  cAPIRef}
               {mfaiset.i cmt_type cAPIType}
               {mfaiset.i cmt_lang cAPILang global_lang}
               cmt_print = cmtPrint.

            do i = 1 to 15:
               if cAPIComment[i] <> ? then
                  cmt_cmmt[i] = cAPIComment[i].
               else if not lLegacyAPI then do:
                  if ttTransactionComment.cmtCmmt[i] = ? then
                     ttTransactionComment.cmtCmmt[i] = cmt_cmmt[i].
               end.
            end.
      ...
      end. /* else do -> cAPIOperation = {&REMOVE} */
   end. /* repeat */
